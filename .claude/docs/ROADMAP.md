...
| Định dạng file dữ liệu | **JSON, đọc bằng `Newtonsoft.Json`** — áp cho mọi file dữ liệu của game: bản đồ, bảng config, bảng item, bảng quái. Lưới ô của map nằm dưới dạng **mảng chuỗi id**, mỗi hàng một dòng, mỗi ô là **id số** của `CellType` (canh cột khi ghi, tách theo khoảng trắng khi đọc). File do **máy sinh** thì bỏ qua trường lạ (`Ignore`), file **người gõ tay** thì trường lạ là lỗi (`Error`) | Chốt ở Phase 10 (2026-08-24); **đổi ký tự `.#=` sang id số 2026-09-08** — ký tự có trần (~vài chục), mà loại ô còn thêm thang/nước/gai/dốc, và ngày file map chứa cả id tile của lớp hình thì một tileset hàng trăm ô là hết cửa ngay. Câu hỏi quyết định là "thêm một trường mới sau này tốn bao nhiêu" — map hôm nay chỉ có lưới va chạm, mai mốt còn cổng sang map khác, danh sách quái, vùng an toàn. Format tự chế thì mỗi trường mới là sửa cả parser lẫn writer; JSON thì thêm một property. Thêm hai cái được miễn phí: số thực không còn dính bẫy locale (spec JSON quy định sẵn, không phải nhớ `InvariantCulture`), và file cũ ↔ code mới ↔ file mới đọc lẫn nhau được. Chọn Newtonsoft vì Unity **đã có sẵn** qua `com.unity.nuget.newtonsoft-json`; `System.Text.Json` nhanh hơn nhưng phải tự kéo DLL vào Unity và hay vướng code stripping của IL2CPP. Trường `version` vẫn giữ — JSON lo được chuyện *thêm* trường, không lo được chuyện *đổi ý nghĩa* trường đã có |
...
| **7** | **Đồng bộ nhiều người chơi** | Mở 2 client (Multiplayer Play Mode), thấy nhau chạy mượt, không giật | Snapshot theo tick · interpolation buffer · vì sao không gửi mỗi frame |
...
| **10 — Map: hình dạng thật** 🆕 | ⏳ **đang làm** — xong Bước 1 (`MapGrid`/`MapFile`/`MapFileData` + CHECKPOINT A); còn Bước 2 (tool export trong Unity) và Bước 3 (thân nhân vật + `Step` va chạm) | [`guides/PHASE-10.md`](guides/PHASE-10.md) ✅ (viết lại 2026-08-24: tách AOI ra Phase 11, pipeline tilemap → file map **JSON**, khớp `CharacterProfile` của Phase 9 · sửa 2026-09-08: `cells` dùng **id số**, thêm khoá `prefab`) |
