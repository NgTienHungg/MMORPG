...
> **Kết quả cuối Phase 11:** tốc độ chạy, điểm spawn, danh sách nghề... nằm trong file `game.json` —
...
>
> **Điều kiện:** xong [`PHASE-10.md`](PHASE-10.md) tới CHECKPOINT B và cả 3 thử nghiệm.
>
...
>
> ⚠️ **Doc này viết khi dự án còn là top-down (khi đó nó mang số 9), và chỉ bàn config *loại A*.**
> Cần bổ sung trước khi làm: (1) `GameConfig` thêm `Gravity`, `JumpForce`, `MaxFallSpeed`,
> `CoyoteTicks`, `JumpBufferTicks` của Phase 8 và các số cooldown / thời lượng trạng thái của Phase 9;
...
- **Map ra file + gửi qua mạng lúc EnterWorld**: map là payload lớn đầu tiên (> 4KB) — dịp để đường nén
  ...
  của Phase 17 (hot update asset).
- **Config từ Google Sheet** (pipeline của `com.hungnt.dataconfig`): xuất sheet → json. Đáng làm khi
  bảng số bắt đầu dày (Phase 12–14: item, quái, damage).

---

**Xong Phase 11 → hết Chặng C.** Thế giới sống: nhiều người thấy nhau, map có hình, số liệu là dữ liệu.
Chặng D bắt đầu vòng gameplay thật — [PHASE-12](PHASE-12.md): túi đồ, feature dọc đầu tiên đi đủ
DB → DAL → logic → packet → UI, khuôn mẫu cho mọi feature về sau. (Viết khi bạn báo xong Phase 11.)
