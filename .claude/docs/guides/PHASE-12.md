...
> **Kết quả cuối Phase 12:** trọng lực, lực nhảy, coyote time, thời lượng đòn đánh, kích thước thân
...
>
> **Điều kiện:** xong [`PHASE-11.md`](PHASE-11.md) tới CHECKPOINT A và cả ba thử nghiệm.
>
...
> ## ⚠️ Doc này CHƯA được soát lại sau Phase 9–11
>
> Bản này viết ngày 2026-08-21, tức là **trước** khi Phase 9 và Phase 10 chốt lại hai thứ mà nó nói tới
> rất nhiều. Ý lớn (hai loại config · chỉ server đọc file · kiểm phiên bản lúc login) vẫn đúng nguyên;
> phần **code và cách đếm số** thì đã lệch. Đọc để lấy ý, đừng chép code:
>
> | Doc này nói | Thực tế sau Phase 9–11 |
> |---|---|
> | Gom **14 hằng số** trong `MovementRules` thành `MovementConfig` | Chỉ còn **6** là hằng toàn cục: `GRAVITY`, `MAX_FALL_SPEED`, `COYOTE_TICKS`, `JUMP_BUFFER_TICKS`, `DROP_THROUGH_TICKS`, `EDGE`. Số riêng của nhân vật (tốc độ, lực nhảy, thời lượng đòn, kích thước thân) **đã nằm trong `CharacterProfile`** từ Phase 9 |
> | `ATTACK_TICKS`, `HURT_TICKS`, `DIE_TICKS` viết bằng **tick** | Phase 9 đã đổi: người thiết kế viết bằng **giây**, `ActionDefinition` quy ra tick một lần lúc dựng bảng |
> | `Step(state, intent, dt, cfg)` | Thực tế sau Phase 10: `Step(state, intent, dt, profile, map)` — và phase này thêm `cfg` là tham số thứ **sáu**, đúng lúc để hỏi lại xem có nên gom `profile` + `map` + `cfg` thành một `SimContext` không |
> | **Bước 4**: đưa bản đồ từ hằng số trong code ra file, kèm hash | **Phase 10 đã làm nửa đầu**: map đã là file text do tool Editor export, đã có `MapGrid.Checksum()`, và hai bên đã in checksum ra console. Việc còn lại của phase này chỉ là **phép so bằng máy lúc login** (gửi checksum trong handshake, lệch thì chặn vào world) — không phải dựng lại pipeline map |
> | Gizmo đọc thẳng file map để đối chiếu hình với luật | Không còn gizmo nào. Lớp luật là một Tilemap vẽ cạnh lớp hình trong cùng Scene; bật/tắt renderer là thấy |
> | Chưa nói gì về thư viện đọc `game.json` | Phase 10 đã chốt cho cả dự án: **JSON + `Newtonsoft.Json`** (Unity có sẵn qua `com.unity.nuget.newtonsoft-json`), DTO khai báo tên trường tường minh bằng `[JsonProperty]`, và tách **DTO của file** khỏi **kiểu chạy trong game**. Riêng phase này dùng `MissingMemberHandling.Error` chứ không `Ignore` như file map — `game.json` là file **người gõ tay**, và ở đó gõ nhầm `gravty` mà im lặng thì trọng lực về mặc định còn bạn đi tìm bug trong `MovementRules` |
>
> Soát lại toàn bộ khi bạn xong Phase 11 — lúc đó mới biết chắc còn bao nhiêu con số và chúng ở đâu.
...
| Làm ở đâu | Bước 1–3 dưới đây | Bước 4 (bản đồ) · bảng item Phase 13 · phân phối qua CDN Phase 18 |
...
- `WorldService`: `AoiColumnWidth` thay `const AOI_COLUMN_WIDTH` (hằng của Phase 11).
...
Nhưng bảng item, bảng quái, bảng chỉ số theo class thì không nhỏ như vậy, và ở Phase 18 chúng đi bằng
...
- **Config từ Google Sheet** (pipeline của `com.hungnt.dataconfig`): xuất sheet → json. Đáng làm khi
  bảng số bắt đầu dày — tức là Phase 13–15 với item, quái, công thức sát thương.
...
- **Tách file config theo môi trường** (`game.dev.json` / `game.prod.json`), chọn bằng biến môi trường.
  ...
  config chưa có gì bí mật, ngày có chuỗi kết nối MySQL ở Phase 20 thì có.
...
**Xong Phase 12 → hết Chặng C.** Thế giới sống: nhiều người thấy nhau ở phạm vi có giới hạn, map có
...
config loại B thứ hai, đi đúng con đường mà bản đồ vừa mở ra hôm nay. (Viết khi bạn báo xong Phase 12.)
