...
> **Kết quả cuối Phase 10:** map có tường thật — client sửa kiểu gì cũng không xuyên được, đi sát tường
...
>
> **Điều kiện:** xong [`PHASE-8.md`](PHASE-8.md) (motor platformer) và [`PHASE-9.md`](PHASE-9.md)
> (state machine trạng thái) tới CHECKPOINT cuối.
>
...
>
> ⚠️ **Doc này viết khi dự án còn là top-down (khi đó nó mang số 8).** Sau khi chốt thể loại platformer
> ngang, ba chỗ cần viết lại trước khi làm: (1) phần va chạm — thêm **sàn xuyên-một-chiều**, bỏ nhánh
> trượt dọc tường theo trục Y, và va chạm phải ăn khớp với `MoveResult` (velocity + grounded) của
> Phase 8 chứ không còn nhận `dirY`; (2) phần AOI — tầm nhìn tính theo **zone + khoảng cách trục X**
> thay vì ô lưới 2D; (3) `MapRenderer` vẽ bằng **Tilemap + tileset American Forest** đã có trong
> `Assets/Game/Textures/`, thay cho ô vuông primitive. Phần diff-tầm-nhìn-mỗi-tick và toàn bộ lập
> luận thì giữ nguyên giá trị.
...
(vẽ hình) cùng đọc **một** định nghĩa — không tồn tại khả năng map hai bên lệch nhau. Phase 11 sẽ đưa
...
**Xong Phase 10 → thế giới có tường, có tầm nhìn, băng thông theo mật độ.** [PHASE-11](PHASE-11.md) trả nốt
